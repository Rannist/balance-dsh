...
.balance-footer{box-sizing:border-box;display:flex;align-items:center;justify-content:flex-start;gap:8px;width:calc(100% + 4px);min-width:0;height:42px;margin:0 -2px;padding:0 10px 0 8px;color:var(--dsw-alias-label-primary);font-size:14px;line-height:22px;text-decoration:none;cursor:pointer;white-space:nowrap;overflow:hidden;-webkit-user-select:none;user-select:none}
...
.balance-footer__value{font-variant-numeric:tabular-nums;font-weight:600;white-space:nowrap;padding:2px 8px;border-radius:999px;margin-left:auto;text-decoration:none;cursor:pointer;transition:transform .15s var(--ds-ease-in-out,.2s)}
...
`;
    ...
    /** 分时段金额：高峰桶按目录价 ×2、空闲桶按目录价（与服务端 estimateCost 口径一致）。 */
    ...
    // ─── 余额显示：展开态侧栏底部文字（¥ 图标+金额）；收起态悬浮 FAB ──
    function BalanceFooter({ wide }) {
      ...
      if (!wide) {
        ...
        const detailText = "总余额 " + b.total_balance + " " + currency + " · 充值余额 " + b.topped_up_balance + " " + currency;
        return jsx.jsx("a", {
      ...
      // 展开态：底部文字（加载中/失败/去充值/正常）
      if (state.status === "error") {
        return jsx.jsx("div", {
          className: "balance-footer",
          title: "余额查询失败：" + state.error,
          onContextMenu: onBalanceContextMenu,
      ...
      const detailText = "总余额 " + totalText + " · 充值余额 " + b.topped_up_balance + " " + currency;
      const period = currentPeriod();
      const low = total > 0 && total < 10;

      // 金额胶囊：正常显示 金额•时段（语义色）；没余额显示 0（红色），并追加"去充值"胶囊
      // 没余额：保留图标+"余额"字样，去掉具体金额，右侧只显示"去充值"按钮
      if (depleted) {
        return jsx.jsx("div", {
          className: "balance-footer",
          onContextMenu: onBalanceContextMenu,
          children: [
            jsx.jsx("span", {
              className: "balance-footer__icon",
      ...
      const valueClass = low ? " balance-footer__value--low" : " balance-footer__value--healthy";
      const valueChildren = [
        totalText,
        " • ",
        state.refreshing
      ...
      const inner = [
        ...
        jsx.jsx("span", { className: "balance-footer__label", children: "余额" }),
        jsx.jsx("a", {
          className: "balance-footer__value" + valueClass,
          href: "https://platform.deepseek.com/usage",
          target: "_blank",
          rel: "noreferrer",
          onContextMenu: onBalanceContextMenu,
