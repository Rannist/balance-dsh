...
.balance-stats-panel{box-sizing:border-box;position:fixed;z-index:10000;width:460px;max-height:min(620px,calc(100vh - 24px));overflow-x:hidden;overflow-y:auto;scrollbar-gutter:stable;padding:16px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-base,#fff);border:1px solid var(--dsw-alias-border-l2,#d9d9df);border-radius:12px;box-shadow:0 14px 36px rgba(0,0,0,.24);transform-origin:left bottom;will-change:transform,opacity;contain:layout style;animation:balance-stats-panel-in .18s var(--ds-ease-in-out,ease-out) both}
.balance-stats-panel--closing{pointer-events:none;animation:balance-stats-panel-out .14s var(--ds-ease-in-out,ease-in) both}
@keyframes balance-stats-panel-in{from{opacity:0;transform:translateY(8px) scale(.975)}to{opacity:1;transform:translateY(0) scale(1)}}
...
.balance-stats-card{min-width:0;padding:11px;border:1px solid var(--dsw-alias-border-l2,#d9d9df);border-radius:10px;background:var(--dsw-alias-bg-module-platform,rgba(127,127,127,.04));animation:balance-stats-card-in .2s var(--ds-ease-in-out,ease-out) both}
...
.balance-heatmap-cell[data-tooltip]{position:relative;cursor:default}
.balance-heatmap-cell[data-tooltip]::after{content:attr(data-tooltip);position:absolute;z-index:3;bottom:calc(100% + 7px);left:50%;max-width:300px;padding:6px 9px;border:1px solid var(--dsw-alias-border-l2,#45454d);border-radius:6px;background:var(--dsw-alias-bg-module-platform,#27272b);box-shadow:0 6px 18px rgba(0,0,0,.22);color:var(--dsw-alias-label-primary,#f4f4f5);font-size:11px;font-weight:400;line-height:1.45;white-space:nowrap;pointer-events:none;opacity:0;transform:translate(-50%,3px);transition:opacity .12s var(--ds-ease-in-out,ease-out),transform .12s var(--ds-ease-in-out,ease-out)}
.balance-heatmap-cell[data-tooltip]:hover::after{opacity:1;transform:translate(-50%,0)}
.balance-heatmap-cell[data-tooltip-edge="left"]::after{left:0;transform:translate(0,3px)}
.balance-heatmap-cell[data-tooltip-edge="left"]:hover::after{transform:translate(0,0)}
.balance-heatmap-cell[data-tooltip-edge="right"]::after{right:0;left:auto;transform:translate(0,3px)}
.balance-heatmap-cell[data-tooltip-edge="right"]:hover::after{transform:translate(0,0)}
...
@media(prefers-reduced-motion:reduce){.balance-stats-panel,.balance-stats-panel--closing,.balance-stats-card{animation:none}.balance-heatmap-cell{transition:none}}
...
`;
    ...
    function UsageStatsPanel({ onClose, panelRef, style, closing }) {
      ...
      const heatmap = react.useMemo(() => {
        ...
        for (let dayIndex = 0; dayIndex < days.length; dayIndex++) {
          ...
          cells.push(jsx.jsx("button", {
            ...
            "data-tooltip-edge": dayIndex < 21 ? "left" : dayIndex >= days.length - 21 ? "right" : "center",
          }, day.date));
        }
        return cells;
      }, [days]);
    ...
    function BalanceFooter({ wide }) {
      ...
      react.useEffect(() => {
        ...
        const closeOnResize = () => closeStats();
