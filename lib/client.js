...
/* ── 跨会话统计面板（单击余额弹出） ── */
.balance-stats-backdrop{position:fixed;inset:0;z-index:10000;display:flex;align-items:center;justify-content:center;padding:20px;background:rgba(15,23,42,.48);backdrop-filter:blur(3px);cursor:default}
.balance-stats-panel{box-sizing:border-box;width:min(820px,calc(100vw - 40px));max-height:min(720px,calc(100vh - 40px));overflow:auto;padding:22px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-base,#fff);border:1px solid var(--dsw-alias-border-l2,#d9d9df);border-radius:16px;box-shadow:0 24px 70px rgba(0,0,0,.28)}
.balance-stats-header{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:18px}
.balance-stats-title{margin:0;font-size:20px;line-height:28px;font-weight:700}
...
.balance-stats-cards{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-bottom:20px}
.balance-stats-card{min-width:0;padding:14px;border:1px solid var(--dsw-alias-border-l2,#d9d9df);border-radius:12px;background:var(--dsw-alias-bg-module-platform,rgba(127,127,127,.04))}
.balance-stats-card__label{display:block;margin-bottom:6px;color:var(--dsw-alias-label-secondary);font-size:12px}
.balance-stats-card__value{display:block;overflow:hidden;color:var(--dsw-alias-label-primary);font-size:18px;font-weight:700;font-variant-numeric:tabular-nums;text-overflow:ellipsis;white-space:nowrap}
...
@media(max-width:680px){.balance-stats-cards{grid-template-columns:repeat(2,minmax(0,1fr))}.balance-stats-panel{padding:16px}.balance-stats-backdrop{padding:10px}}
...
`;
    ...
    function UsageStatsPanel({ onClose }) {
      ...
      return jsx.jsx("div", {
        className: "balance-stats-backdrop",
        role: "presentation",
              ...
              children: [
                jsx.jsxs("div", {
                  children: [
                    ...
                    jsx.jsx("p", { className: "balance-stats-subtitle", children: "单击查看统计 · 双击余额打开 DeepSeek API 用量页" }),
                  ],
                }),
    ...
    function BalanceFooter({ wide }) {
      ...
      const withPanel = (node) => jsx.jsxs(jsx.Fragment, {
        children: [node, statsOpen ? jsx.jsx(UsageStatsPanel, { onClose: () => setStatsOpen(false) }) : null],
      });
