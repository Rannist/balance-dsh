...
.balance-stats-panel{box-sizing:border-box;position:fixed;z-index:10000;width:460px;max-height:min(620px,calc(100vh - 24px));overflow:auto;padding:16px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-base,#fff);border:1px solid var(--dsw-alias-border-l2,#d9d9df);border-radius:12px;box-shadow:0 14px 36px rgba(0,0,0,.24);transform-origin:left bottom;will-change:transform,opacity;contain:layout style;animation:balance-stats-panel-in .18s var(--ds-ease-in-out,ease-out) both}
...
.balance-heatmap-cell{width:14px;height:14px;border-radius:3px;background:rgba(127,127,127,.10);transition:transform .12s var(--ds-ease-in-out,ease-out),filter .12s var(--ds-ease-in-out,ease-out)}
    ...
    function UsageStatsPanel({ onClose, panelRef, style, closing }) {
      ...
      const heatmap = react.useMemo(() => {
        ...
        for (const day of days) {
          ...
          const ariaLabel = day.date + "，Token " + formatTokens(tokens) + "，费用 " + costText + "，会话 " + sessionsText;
          const tooltip = day.date + " · Token " + formatTokens(tokens) + " · 费用 " + costText + " · 会话 " + sessionsText;
          cells.push(jsx.jsx(primitives.Tooltip, {
            label: tooltip,
            side: "top",
            delayMs: 100,
            maxWidth: 320,
